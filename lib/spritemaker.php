<?php
	...
	class spritemaker
	{
		...
		function __construct( $name , $settings , $outdir , $outurl , $versionnum )
		{
		...
		function makePaths( $name , $outdir , $outurl , $versionnum )
		{
		...
		function makeSprites()
		{
			...
			if ( $this->settings->width ) { $this->css[ $this->settings->className ]['width'] = $this->settings->width.'px'; }
			if ( $this->settings->height ) { $this->css[ $this->settings->className ]['height'] = $this->settings->height.'px'; }
			...
			foreach ( $this->settings->input as $i => $directory )
			{
				...
				if ( $directory->className )
				{
					...
					if ( $directory->width ) { $this->css[ $directory->className ]['width'] = $directory->width.'px'; }
					if ( $directory->height ) { $this->css[ $directory->className ]['height'] = $directory->height.'px'; }
				}
		...
		private function readDirectory( $path )
		{
			...
			foreach( $dirContents as $filename )
			{
				...
				$ext = array_pop($parts);
				 ...
				 //Ignore non-images
				if ( $ext != 'jpg' && $ext != 'png' && $ext != 'gif' ) { continue; }
		...
		private function makeClassNameFromFilename( $filename )
		{
			...
			while ( isset($this->css[$className]) ) //If this name is already in use
			{
				++$i;
